...
**提示词设计：** 系统提示词将 AI 定位为"HelpMate 校园跑腿互助平台的智能客服"，要求用简洁友好的语言回答，若话题与平台无关则礼貌引导用户回到平台相关话题。提示词按业务模块分节，涵盖平台简介、账号相关（注册字段、登录 Token 有效期24小时）、任务相关（发布流程与赏金冻结逻辑、取消限制、四种任务状态说明）、接单相关（筛选浏览、接单限制、确认完成与赏金结算、取消订单流程）、钱包相关（余额查询、充值与提现规则、五类流水说明）、评价相关（1-5星+文字、每人只能评一次、综合评分计算）、聊天相关（会话列表、未读数）、通知相关（接单/完成/取消/评价等系统通知），以及常见问题汇总（余额不足、接单后无法单方删除、只有发布者可确认完成等）。详细的分节内容使 AI 能够精准回答用户各类操作问题，减少无效引导。
